...
;(function (global) {
  ...
  function paymentApiOrigin() {
    ...
    if (isGitHubPages()) {
      const remote = String(configDefaults().apiBase || localStorage.getItem(API_BASE_KEY) || '')
        .trim()
        .replace(/\/$/, '')
      return remote
    }
    if (isLocalDev() && location.port === '8787') return location.origin
    if (isVercelHost()) return location.origin
  ...
  function resolveProxyHealth() {
    const origin = paymentApiOrigin()
    if (isGitHubPages() && origin) return `${origin}${API_HEALTH}`
    return API_HEALTH
  }
